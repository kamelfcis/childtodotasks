  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl border-2 ${colors.border} overflow-hidden shadow-lg ${colors.shadow}`}
      >
        {/* Child Header */}
        <div className={`bg-gradient-to-r ${colors.header} p-3 sm:p-5 border-b ${colors.border}`}>
          {/* Large avatar */}
          <div className="flex justify-center mb-2 sm:mb-3">
            <motion.div
              className="w-28 h-28 sm:w-36 sm:h-36 md:w-40 md:h-40 rounded-2xl bg-white border-2 sm:border-3 border-white/80 overflow-hidden shadow-lg"
            ...
            >
              {child.avatar_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-pink-100 to-blue-100">
                  <FiUser className="text-3xl sm:text-4xl text-purple-300" />
                </div>
              )}
            </motion.div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
                ...
                {earnedPoints > 0 && (
                  <AnimatePresence>
                    <motion.span
                      ...
                      className="text-neon-green font-black text-xs sm:text-sm"
                    >
                      +{earnedPoints} ⭐
                    </motion.span>
                  </AnimatePresence>
            ...
            <Link to={`/child/${child.id}`}>
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="p-2.5 sm:p-3 rounded-xl bg-white/80 border border-purple-200 text-text-muted hover:text-neon-purple hover:border-neon-purple/40 transition-all shadow-sm"
        ...
        <div className="p-3 sm:p-4 space-y-2 sm:space-y-3">
          {loading ? (
            ...
            <div className="py-4 sm:py-6 text-center">
              ...
              <p className="text-text-muted text-xs sm:text-sm">No tasks yet today</p>
            </div>
          ) : (
        ...
        {completedCount === totalCount && totalCount > 0 && (
          <motion.div
            ...
            transition={{ type: 'spring', damping: 12 }}
            className="mx-3 sm:mx-4 mb-3 sm:mb-4 text-center bg-gradient-to-r from-green-50 to-blue-50 rounded-xl sm:rounded-2xl p-4 sm:p-5 border-2 border-green-200"
          >
            <motion.div
            ...
            <h3 className="text-base sm:text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-neon-green to-neon-blue">
              ALL DONE!
            </h3>
            <p className="text-text-muted text-xs sm:text-sm mt-1 font-semibold">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8 relative z-10">
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-text-muted mt-2 text-sm">Go to Dashboard to add your children first!</p>
            <Link to="/dashboard">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="mt-4 sm:mt-6 px-5 sm:px-6 py-2.5 sm:py-3 rounded-2xl bg-gradient-to-r from-neon-pink to-neon-purple text-white font-bold shadow-lg shadow-pink-200 text-sm sm:text-base"
              >
                Go to Dashboard →
              </motion.button>
            </Link>
